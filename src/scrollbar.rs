...
pub fn draw(builder: &mut ScrollBarBuilder, renderer: &sdl2::render::Renderer) -> bool {
	...
	//println!("{}, {}, {}, {}", value - builder.min_value, (value - builder.min_value) / value_range, ((value - builder.min_value) / value_range * place_count as f32), ((value - builder.min_value) / value_range * place_count as f32) as i32);
	...
	//imgui::fill_rect(renderer, min_label_w+char_w + x + char_w*value_place, y, 10, char_h*2, RGB(82, 85, 90));
	...
	if hover {
		...
		let str = builder.value.to_string();
	...
	let hover_pointer = builder.layer.is_mouse_in(pointer_x, y, char_w, h);
	let click = builder.layer.is_mouse_down() && hover;
	//let just_clicked = click && !active;
